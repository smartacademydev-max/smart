...
import { useChangeBundleStatusMutation, useDeleteBundleMutation, useGetAllBundleQuery } from "../../../../../services/questionApi";
...
export default function AllSets() {
    ...
    const [bundleToDelete, setBundleToDelete] = useState<string[]>([]);
    ...
    const openDeleteConfirmation = (selectedCourseIds: string[]) => {
        setBundleToDelete(selectedCourseIds);
        setOpenConfirm(true);
    };
    ...
    const columns = useMemo<ColumnDef<SetProps>[]>(() => [
        {
            header: () => (
                <Stack sx={{ gap: "10px" }}>
                    <Checkbox
        ...
        },
        {
            ...
            cell: ({ row }) => (
                <Actions
                    deleting={deleting}
                    editUrl={PATH.SET.EDIT_SET.ROOT(Number(row.original.id))}
                    viewUrl={PATH.SET.VIEW_SET.ROOT(Number(row.original.id))}
    ...
    return (
        <div className="all__set__root h-full flex flex-col justify-between">
            <div className="page__top">
                <PageHeader
                    ...
                    cta={
                        {
                            icon: <Add />,
                            url: PATH.SET.CREATE_SET.ROOT,
                            label: t("messages.empty_states.bundle.action"),
                        }
                    }
                />
