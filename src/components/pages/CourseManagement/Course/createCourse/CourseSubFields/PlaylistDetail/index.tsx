...
export default function PlaylistDetailPage() {
    const { id, playlistId, type } = useParams<{
        id?: string;
        playlistId?: string;
        type?: courseTabType;
    }>();
    ...
    const { data, isLoading } = useGetSinglePlaylistQuery(
        {
            id: id ? Number(id) : 0,
            playlistId: playlistId ? Number(playlistId) : 0,
            type: (type || "videos") as courseTabType,
            pageIndex: qp.pageIndex,
            pageSize: qp.pageSize,
            search: "",
        },
        { skip: !id || !playlistId || !type }
    );
    ...
    const getTypeConfig = () => {
        ...
        return configs[type || "videos"];
    };

    const config = getTypeConfig();

    return (
        <div className="playlist__detail__page ">

            {!isLoading && !mediaItems?.length && (
                <EmptyRoute
                    title="No Media Found"
                    message="This playlist doesn't contain any media items yet."
                    variant="warning"
                    icon={config?.icon}
                />
            )}
            ...
            <div className="flex-1 overflow-auto">
                <div className="flex flex-col gap-4 md:grid grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 mb-6">
                    {isLoading ? (
                        [...Array(8)].map((_, idx) => (
                            <div key={idx} className="col-span-1">
                                <Box className="w-full">
                                    <Skeleton variant="rectangular" height={200} className="rounded-md mb-2" />
                        ...
                        mediaItems.map((media) => (
                            <div key={media.id} className="col-span-1">
                                <MediaCard media={media} type={type as any} />
                            </div>
                        ))
                    )}
                </div>
