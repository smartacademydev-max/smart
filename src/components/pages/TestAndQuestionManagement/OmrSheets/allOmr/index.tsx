...
import { useDeleteOmrSheetMutation, useGetAllOmrQuery } from "../../../../../services/questionApi";
...
export default function AllOmrSheets() {
    ...
    const [omrsToDelete, setOmrsToDelete] = useState<string[]>([]);
    useEffect(() => {
    ...
    const handleSelectAll = (checked: boolean) => {
        if (checked) {
            const allIds = new Set(Omrs.map((test) => test.id || ''));
    ...
    const isAllSelected = Omrs.length > 0 && selectedRows.size === Omrs.length;
    const isSomeSelected = selectedRows.size > 0 && selectedRows.size < Omrs.length;

    const openDeleteConfirmation = (selectedCourseIds: string[]) => {
        setOmrsToDelete(selectedCourseIds);
        setOpenConfirm(true);
    };

    const handleOMRDeletion = async () => {
        try {
            const response = await deleteOmr({
                body: omrsToDelete,
            }).unwrap();
    ...
    const columns = useMemo<ColumnDef<OmrSheetProps>[]>(() => [
        {
            header: () => (
                <Stack sx={{ gap: "10px" }}>
                    <Checkbox
    ...
    const handleOmrClose = () => {
        setOpen(false);
        setSelectedOmr(undefined);
    }
    return (
        <div className="all__omr__sheets  h-full flex flex-col justify-between">
            <div className="page__top">
                <PageHeader
                    breadcrumb={[
                        {
                            title: t("menus.test_question_management.test.omr.root"),
                    ...
                    cta={
                        {
                            ...
                            icon: <Add />,
                            label: t("messages.empty_states.omr.action"),
                        }
                    }
                />
            ...
            {!isLoading && !Omrs.length ?
                <EmptyRoute
                    title='OMR Not Found'
                    message='Oops your OMr is empty. Please add question to help student gain knowlegde.'
                    icon={(
